...
API_REFERENCE = (
    ...
    ("Find many / choose", (
        ("findAllList", 'for m in findAllList("row.png"):', "Every match as a list sorted best-first (no error if none)"),
        ("findAllByRow", 'findAllByRow("cell.png")', "All matches ordered top-down then left-right; findAllByColumn for the other order"),
        ("findBest", 'findBest("ok.png", "yes.png")', "Best single match among several pictures"),
        ("findAny", 'findAny("a.png", "b.png")', "List of whichever of the given pictures are on screen now"),
        ("waitAny", 'waitAny(10, "ok.png", "err.png")', "Wait until any of the pictures appears; waitBest returns the strongest"),
        ("has", 'if has("popup.png"):', "True/False shortcut for exists()"),
        ("Finder", 'f = Finder("scene.png")', "Search inside an image file (not the live screen): .find .findAll .next .findChanges"),
    )),
    ("Screen text (OCR)", (
        ("read region", "Region(0, 0, 800, 200).text()", "Read all text inside a screen area as one string"),
        ("findText", 'm = findText("Save As")', "Find a line of text on screen and return a clickable Match; error if missing"),
        ("findWord", 'findWord("Username")', "Find a single word; findLine finds a whole line"),
        ("hasText / waitText", 'if hasText("Ready"):', "hasText returns True/False; waitText waits for text to appear"),
        ("findWords", 'for m in findWords():', "Every recognised word as a Match (with .getText()); findLines groups them into lines"),
        ("collectLinesText", "lines = collectLinesText()", "All lines as plain strings; collectWordsText for words"),
        ("OCR.readText", 'OCR.readText("crop.png")', "OCR any image file/region; readLine readWord readChar readWords readLines"),
        ("OCR language", 'OCR.language("tur")', "Switch OCR language; same as Settings.OcrLanguage (bundled: eng, eng_best, tur, dejavu_sans)"),
        ("OCR engine", "engine = configured_ocr()", "Full OCR engine with word boxes and languages"),
    )),
    ("Watching for events (observe)", (
        ("onAppear", 'reg.onAppear("alert.png", handler)', "Register: call handler when the picture appears in this region"),
        ("onVanish", 'reg.onVanish("spinner.png", handler)', "Register: call handler when the picture disappears"),
        ("onChange", "reg.onChange(100, handler)", "Register: call handler when at least N pixels change in the region"),
        ("observe", "reg.observe(30)", "Watch for up to 30s (blocking); observeInBackground(30) runs in a thread"),
        ("handler", "def handler(e): print(e.getType())", "Handler gets an ObserveEvent: .getMatch() .getRegion() .getChanges() .repeat()"),
        ("stopObserver", "reg.stopObserver()", "Stop a running background observation"),
        ("getEvents", "for e in reg.getEvents():", "Pull the events that fired; hasEvents/isObserving report state"),
    )),
    ...
    ("Dialogs and input", (
        ("popup", 'popup("done!")', "Show a message box with an OK button"),
        ("popAsk", 'if popAsk("Continue?"):', "Yes/No box; returns True when the user clicks Yes"),
        ("popError", 'popError("Something broke")', "Message box with an error icon"),
        ("input", 'name = input("Your name?")', "Ask for a single line of text (hidden=True for passwords)"),
        ("inputText", 'notes = inputText("Notes:")', "Ask for multi-line text"),
        ("select", 'choice = select("Pick", options=["A", "B"])', "Drop-down list; returns the chosen item"),
        ("popFile", "path = popFile()", "Open-file dialog; returns the chosen path"),
    )),
    ("Flow and output", (
        ("sleep", "sleep(2)", "Wait a number of seconds"),
        ("setShowActions", "setShowActions(True)", "Flash a marker at each click/drag before it happens (demo mode)"),
        ("exit", "exit(0)", "Stop the script now with an exit code"),
